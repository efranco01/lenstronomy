...
__all__ = ["PJaffe_Ellipse_Mass"]

class PJaffe_Ellipse_Mass(LensProfileBase):
    """
    ...
    with the ellipticity (flattening) implemented in the convergence/mass

    Module name: 'PJAFFE_ELLIPSE_MASS'';
    ...
    lower_limit_default = {
        "sigma0": 0, # lower limit on central surface mass density
        "Ra": 0, # lower limit on scale radius along major axis
        "Rs": 0, # lower limit on scale radius along minor axis
        "e1": -0.5, # lower limit on ellipticity component 1
        "e2": -0.5, # lower limit on ellipticity component 2
        "center_x": -100, # lower limit on x-position of center
        "center_y": -100, # lower limit on y-position of center
    }
    upper_limit_default = {
        "sigma0": 10, # upper limit on central surface mass density
        "Ra": 100, # upper limit on scale radius along major axis
        "Rs": 100, # upper limit on scale radius along minor axis
        "e1": 0.5, # upper limit on ellipticity component 1
        "e2": 0.5, # upper limit on ellipticity component 2
        "center_x": 100, # upper limit on x-position of center
        "center_y": 100, # upper limit on y-position of center
    }

    def __init__(self):
        self.spherical = PJaffe()
        self._diff = 0.000001
        super(PJaffe_Ellipse_Mass, self).__init__()

    def function(self): # Lensing Potential With Ellipticity.
        """Returns a non-distoring lensing potential.
        :param E_0 = 2 * w * k_0: 
        :param w: 
        :param e: ellipticity
        :param r_em: r_em^2 = /frac{x^2}{(1+e)^2} + /frac{y^2}{(1-e)^2}
        :param K: 
        """
